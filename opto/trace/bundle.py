...
class FunModule(Module):
    ...
    def preprocess_code(self, raw_code):
        """
        Preprocesses the code to handle line continuation characters while preserving comments.
        """
        ...
        for line in lines:
            ...
            if stripped or continuation:
                if stripped.endswith('\\'):
                    continuation = True
                    cleaned_lines.append(stripped[:-1])  # Remove single backslash
                else:
                    continuation = False
        ...
        # Final cleanup of any remaining escaped characters
        output = output.replace('\\\\', '\\')  # Handle escaped backslashes
        # print("CLEANED CODE")
        # print(output)
        return output
